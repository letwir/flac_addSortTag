# flac_addSortTag
## 概要
flacファイルに、**読み仮名ベースのソートタグ**（カタカナ）を追記するツールです。  
  
## 事の発端
音楽ファイルをfoobar2000やiTunesで管理していると、アーティスト名のフリガナが無い曲のソートが不便だったので作った。
...
なるべくOS依存を避けるため、取得手順はWindows / Linuxで共通化しています。
git cloneでも可。
### 実行すると起こること
カナ変換したものを各sortタグに上書きします。
 * 【読み込みタグ】 -> 【書き込みタグ】
 * TITLE -> titlesort  
 * ARTIST -> artistsort  
